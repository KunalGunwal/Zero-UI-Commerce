...
![Database](https://img.shields.io/badge/Database-DynamoDB-FF9900.svg)
...
**Zero-UI Commerce** is a multimodal, conversational quick-commerce architecture designed to eradicate the friction of traditional GUI-based shopping. It replaces search bars and complex navigation with a zero-learning-curve WhatsApp interface powered by LLMs. 

Users can simply send a voice note or a Hinglish text message, and the system instantly translates their intent into a fully-priced, payment-ready shopping cart.
...
* **🛒 "My Usual" Memory Ledger:** Persistent database state management allows users to instantly reload past orders with a single command.
* **📱 Bi-Directional Partner Dashboard:** A secure, React-styled command console featuring real-time, tri-phase data aggregation (Active Carts → Dispatch Queue → Fulfilled Archives) and automated WhatsApp delivery tracking pings.
...
flowchart TD
    ...
    classDef db fill:#232F3E,stroke:#FF9900,stroke-width:2px,color:white,font-weight:bold;
    ...
    subgraph Cloud_Ecosystem [Cloud & Data Layer]
        direction LR
        LLM_API[(LLM Engine / Gemini)]
        DDB[(DynamoDB)]
        CATALOG[(Inventory API)]
    end

    WA <-->|Audio/Text Payload| WH
    LLM <-->|Audio Transcription & NLP| LLM_API
...
* Python 3.9+
* Cloud Database access (e.g., DynamoDB)
...
```bash
git clone [https://github.com/yourusername/zero-ui-commerce.git](https://github.com/yourusername/zero-ui-commerce.git)
cd zero-ui-commerce
...
DB_ACCESS_KEY_ID=your_access_key
DB_SECRET_ACCESS_KEY=your_secret_key
...
Run the seeder script to create the database tables and populate the massive grocery catalog:
...
The Partner Console is protected via HTTP Basic Auth to secure customer logistics data.
